...
const CommentSection: React.FC<CommentSectionProps> = ({ video, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm transition-opacity">
      <div className="bg-[#1a1a1a] w-full sm:w-[400px] sm:h-[600px] h-[70vh] rounded-t-2xl sm:rounded-xl flex flex-col overflow-hidden animate-slide-up">
        
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-gray-800">
          <div className="w-6" /> {/* Spacer */}
          <h3 className="font-semibold text-sm">{comments.length} comments</h3>
          ...
          {comments.map((comment) => (
            <div key={comment.id} className="flex gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs text-gray-400 font-medium">{comment.username} • {comment.timestamp}</p>
        ...
        <div className="p-3 border-t border-gray-800 bg-[#1a1a1a]">
          ...
          <div className="flex gap-2 mb-2 overflow-x-auto no-scrollbar">
             <button 
               ...
               className="flex items-center gap-1 bg-gradient-to-r from-purple-600 to-pink-600 text-xs px-3 py-1.5 rounded-full font-medium whitespace-nowrap disabled:opacity-50"
             >
               <Sparkles size={12} />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 relative">
              <input
                ...
                className="w-full bg-gray-800 text-sm px-4 py-2.5 rounded-full outline-none focus:ring-1 focus:ring-gray-600 transition-all pr-10"
                onKeyDown={(e) => e.key === 'Enter' && handleSend()}
              />
              <button 
                onClick={handleSend}
                className={`absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full ${newComment.trim() ? 'text-pink-500' : 'text-gray-500'}`}
              >
                <Send size={16} />
              </button>
            </div>
